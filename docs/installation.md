...
They can deliver unreleased changes, including the 60-workflow inventory
...
Payday Super tools when they are available and fall back to their own source
checks when they are not. To run the skills without the server, disable it with
...
The default `main` branch contains 60 skills, including
...
overwriting skills already installed there. The command copies all 60 skills
