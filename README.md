...
Install all 55 workflows from the plugin marketplace. This route delivers the `main` inventory, the 55-skill development set preparing v0.3.0, not a published release:
...
That copies the 19 released skills into the current project's Codex and Claude Code directories. It does not change a global installation. The [Codex plugin, the `npx skills` route, manual copying and versioning](docs/installation.md) cover the rest. The 36 skills `main` adds to the tag have no recorded model evaluation.
...
The development installation resolves the default branch and adds 36 workflows to v0.2.1. See the [Australian topic coverage map](docs/au-guide-coverage.md) for the additions and verification limits, and [Installation](docs/installation.md) for the Hardhat Ledger collision warning.
...
- [Fifty-five skills and their supporting files](docs/skill-catalogue.md)
