...
Reconcile the payroll year 3 ways (register, GL, STP-filed) and then finalise. **Human review pending:** the finalisation record was repointed on 24 September 2026 from a URL that returned HTTP 404, and a person has not yet re-dated it. An agent check on 23 September 2026 (not a human review; see `docs/source-review-2026-09-23.md`) found the [ATO end-of-year finalisation page](https://www.ato.gov.au/businesses-and-organisations/hiring-and-paying-your-workers/single-touch-payroll/start-reporting/end-of-year-finalisation-through-stp) (QC 58561, last updated 16 October 2025) stating 14 July for arm's length employees, 30 September for closely held payees, and the payee's tax return due date for a small employer (19 or fewer employees) with only closely held payees. Before classifying a file as late, reverify the current deadline and closely held payee concessions at a reachable ATO source. Record its title, direct URL, check date, applicable year and exact deadline; otherwise keep the timing outcome `UNKNOWN` for human review.
...
1. **Register versus GL.** Reconcile wage and super expense totals to the payroll register. Reconcile each liability separately: opening PAYG/SG payable + current-year payroll liability − payments, reversals and supported adjustments = closing GL payable. Do not compare annual PAYG withheld or SG expense directly with a closing liability. Wages clearing should nil out; explain any residue.
2. **Register versus STP.** Per-employee YTD gross, tax, and super in the software's STP summary agree to the register. **Human review pending:** an agent check on 23 September 2026 (not a human review; see `docs/source-review-2026-09-23.md`) found the [ATO STP reporting under Payday Super page](https://www.ato.gov.au/businesses-and-organisations/super-for-employers/paying-super-on-payday/single-touch-payroll-reporting-under-payday-super) (QC 107592, last updated 27 June 2026) requiring qualifying earnings and super liability amounts for paydays from 1 July 2026, with reports lacking both rejected from 1 July 2027. That page names no field codes or specification version. **UNVERIFIED:** the field names and codes. Retrieve the applicable ATO STP specification and record its version, field names, commencement date, URL and check date before requiring those fields. Reconcile them only where the verified specification applies; otherwise flag `UNKNOWN`, not a filing defect. Investigate every difference. Common causes: unfiled pay events, post-filing adjustments, employees paid outside payroll.
...
4. **Categories and codes.** STP Phase 2 disaggregation: allowances in their proper categories, salary sacrifice reported correctly, ETPs coded per type. Spot-check unusual payees. Use the field specification verified in step 2 for payday-super fields and codes. Keep unsupported requirements `UNKNOWN`.
