...
Reconcile the payroll year 3 ways (register, GL, STP-filed) and then finalise. The recorded 14 July deadline is **UNVERIFIED for the current cycle**: the indexed ATO finalisation URL returned HTTP 404. Before classifying a file as late, verify the current deadline and closely held payee concessions from a reachable ATO source. Record its title, direct URL, check date, applicable year and exact deadline; otherwise keep the timing outcome `UNKNOWN` for human review.
