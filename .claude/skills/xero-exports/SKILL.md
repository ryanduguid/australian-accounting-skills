...
2. Totals, subtotals and running balances in the inspected `.xlsx` exports use live formulas, including `SUM`, direct references and arithmetic. All 2,005 original formula caches in the supplied 5 September 2026 exports were zero. Excel recalculates on open; pandas, openpyxl and any other reader that takes those cached values sees zero. Recompute from the detail rows, or save a recalculated copy from Excel before reading it elsewhere. Recheck caches in later exports rather than assuming they have the same values.
